...
const updateUserPassword = async(req, res, next) => {
    ...
    try {
        ...
        const isValidPayload = userController.validatePasswordUpdatePayload(payload);
        ...
        log.info('Call controller function to update user password');
        const updatedUser = await userController.updateUserPassword(userId, payload);
        ...
        if (err.resType === 'INTERNAL_SERVER_ERROR') {
            log.error('Internal Error occurred while working with update user password router function');
        } else {
